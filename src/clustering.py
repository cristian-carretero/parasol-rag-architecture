...
def normalize_curve(jv: pd.DataFrame, n_points: int = 50) -> np.ndarray:
    """
    Normalizes a single J-V sweep to [0,1] range to isolate morphological features 
    (pseudo-FF, hysteresis) from absolute irradiance magnitude.
    Returns a 1D array of length n_points*2 (Reverse + Forward sweeps).
    """
    # Extract to NumPy immediately to avoid Pandas overhead in deep loops
    ...
    # Prevent division by zero on corrupted flat sweeps
    if v_range == 0 or j_range == 0:
        return np.full(n_points * 2, np.nan)

    # Detect physical turnaround point (Voc approximate)
    ...
    def interpolate_sweep(v_sweep, j_sweep):
        ...
        # Native NumPy averaging for duplicated sensor readings (highly optimized)
...
def normalize_dataset(jv_df: pd.DataFrame, n_points: int = 50) -> pd.Series:
    """
    Applies global shape normalization to the entire dataset using optimized 
    multiprocessing to prevent GIL locks and memory explosions.
    """
    logger.info("Applying full hysteresis shape normalization (Parallelized via loky)...")
    
    # Evaluate as a generator to prevent memory overflow
    grouped = jv_df.groupby(['cell_id', 'curve'])
    keys: List[Tuple[Any, Any]] = [
        cast(Tuple[Any, Any], key) for key in grouped.groups.keys()
    ]
    
    # Use loky backend to bypass Python GIL constraints with Pandas objects
    results: List[np.ndarray] = cast(
        List[np.ndarray],
        list(
            Parallel(n_jobs=-1, backend="loky")(
                delayed(normalize_curve)(df, n_points) for _, df in grouped
            )
        ),
    )
    ...
    # Filter structurally invalid interpolations
...
def stratified_sample_by_cell(curves_series: pd.Series, max_samples: int) -> pd.Series:
    """
    Performs balanced stratified sampling to prevent highly prolific devices 
    from skewing the PCA/K-Medoids topological space.
    """
...
def evaluate_cluster_metrics(
    X_pca_vals: np.ndarray, pca_model: PCA, n_points: int = 50, 
    max_k: int = 8, sample_size: int = 2000
) -> Tuple[List[int], List[float], List[float], List[float], List[float], List[float]]:
    """
    Evaluates optimal K using statistical bounds and a custom Thermodynamic 
    Separation metric (Min ΔpFF) to ensure clusters represent distinct physical states.
    """
...
def get_optimal_pca_components(X: np.ndarray, target_variance: float = 0.90) -> int:
    ...
    reached_target = np.flatnonzero(cum_var >= target_variance)
    if len(reached_target) == 0:
        optimal_comps = len(cum_var)
        logger.warning(
            "PCA target variance %.1f%% was not reached; using all %d available components.",
            target_variance * 100,
            optimal_comps,
        )
    else:
        optimal_comps = int(reached_target[0] + 1)
        logger.info(
            f"PCA dynamic selection: {optimal_comps} components capture >= {target_variance*100}% variance."
        )
    return optimal_comps


def get_optimal_k(
    X_pca: np.ndarray, pca_model: PCA, max_k: int = 8, 
    sample_size: int = 2000, min_valid_k: int = 3, min_dpff_thresh: float = 0.04
) -> int:
    """
    Intelligent K-selection algorithm combining statistical consensus (Elbow, Silhouette, DB) 
    with Thermodynamic Thresholding to reject redundant physical states.
    """
    ...
    # Normalized Geometric Elbow vector calculation
    ...
    print("\n" + "=" * 60)
    print(" [DIAGNOSTICS] AUTOMATED TOPOLOGICAL K-SELECTION EVALUATION")
    print("=" * 60)
    print(f" {'K':<4} | {'Inertia (↓)':<13} | {'Elbow Dist (↑)':<15} | {'Silhouette (↑)':<15} | {'DB Index (↓)':<15} | {'Min ΔpFF (↑)':<12}")
    print("-" * 60)
    
    for i, k in enumerate(k_range):
        flag = " *" if (k >= min_valid_k and min_dpff[i] >= min_dpff_thresh) else "  "
        print(f" {k:<4}{flag}| {inertia[i]:<13.2f} | {dists_to_line[i]:<15.4f} | {silhouette[i]:<15.4f} | {davies[i]:<15.4f} | {min_dpff[i]:<12.4f}")
    print("=" * 60)

    valid_indices = [i for i, k in enumerate(k_range) if k >= min_valid_k and min_dpff[i] >= min_dpff_thresh]
    
    if not valid_indices: 
        logger.warning("No evaluated K satisfies thermodynamic separation. Falling back to geometric optimum.")
    ...
    final_k = k_range[best_elbow_idx]
    consensus = False

    if k_range[best_sil_idx] == k_range[best_db_idx] and k_range[best_sil_idx] != final_k:
        final_k = k_range[best_sil_idx]
        consensus = True

    print(" [DECISION LOGIC]")
    print(f" -> Optimal Geometric Elbow : {k_range[best_elbow_idx]}")
    print(f" -> Max Silhouette Score    : {k_range[best_sil_idx]}")
    print(f" -> Min Davies-Bouldin      : {k_range[best_db_idx]}")
    print("-" * 60)
    print(f" >> FINAL SELECTED k        : {final_k} {'(Statistical Override)' if consensus else '(Elbow Anchor)'}")
    print("=" * 60 + "\n")
...
def analyze_cluster_medoids(centers_orig: np.ndarray, n_points: int = 50) -> Tuple[pd.DataFrame, List[int]]:
    """
    Applies Thermodynamic Boundary Rules to extracted cluster medoids to identify 
    and prune physically impossible or corrupted topologies.
    """
    v_norm = np.linspace(0, 1, n_points)
    metrics, invalid_clusters = [], []
    
    print("\n" + "=" * 60)
    print(" [XAI DIAGNOSTICS] THERMODYNAMIC MEDOID VALIDATION (PRUNING)")
    print("=" * 60)
    ...
    for i, center in enumerate(centers_orig):
        ...
        # Rule 1: Photocurrent near maximum at short-circuit
        j_sc = j_rev[0]
        rule1_fail = j_sc < 0.50  
        
        # Rule 2: Net current near zero at open-circuit
        j_oc = j_rev[-1]
        rule2_fail = j_oc > 0.30
        
        # Rule 3: Monotonicity (Current should not generally increase with voltage)
        dj_dv = np.diff(j_rev) / (np.diff(v_norm) + 1e-9)
        positive_slope_ratio = np.mean(dj_dv > 0.05) 
        rule3_fail = positive_slope_ratio > 0.15
...
def train_kmedoids_pipeline(
    ...
    """
    Executes the unsupervised pipeline: PCA -> Isolation Forest -> K-Medoids -> Pseudo-FF Reordering.
    Evaluates complete hysteresis features.
    """
    ...
    if len(curves_normalized) > max_train_samples:
        ...
        logger.info(f"Stratified sampling enforced: {len(curves_train)} training curves extracted.")
    else:
    ...
    # Keep the full model for variance/loadings diagnostics.
    ...
    if pca_model is None:
        ...
        pca = pca_model
        if n_pca_components is not None and pca.n_components_ != n_pca_components:
            raise ValueError("'pca_model' and 'n_pca_components' specify different component counts.")
    ...
    if len(X_all_pca_cleaned) > max_train_samples:
        train_idx = np.random.RandomState(42).choice(len(X_all_pca_cleaned), size=max_train_samples, replace=False)
        X_train_pca = X_all_pca_cleaned[train_idx]
    else:
        X_train_pca = X_all_pca_cleaned

    if n_clusters is None:
        raise ValueError("Parameter 'n_clusters' must be specified.")

    kmedoids = KMedoids(n_clusters=n_clusters, random_state=42, method='pam')
    kmedoids.fit(X_train_pca)

    # Physics-based Reordering (Ranked by Pseudo Fill-Factor)
    v_norm = np.linspace(0, 1, n_points)
    cluster_centers = kmedoids.cluster_centers_
    if cluster_centers is None:
        raise RuntimeError("K-Medoids did not produce cluster centers.")
    centers_orig = pca.inverse_transform(cluster_centers)
    
    pseudo_ff = [(np.max(v_norm * c[:n_points]) + np.max(v_norm * c[n_points:])) / 2 for c in centers_orig]
    sorted_idx = np.argsort(pseudo_ff)[::-1]
    label_map = {old: new for new, old in enumerate(sorted_idx)}

    labels_all_raw = kmedoids.predict(X_all_pca_cleaned)
    labels = np.array([label_map[l] for l in labels_all_raw])
    kmedoids.cluster_centers_ = cluster_centers[sorted_idx]
    
    centers_orig_sorted = centers_orig[sorted_idx]
    ...
    # Auto-pruning physically invalid curves
    for inv_c in invalid_clusters:
        labels[labels == inv_c] = -1
        
    if invalid_clusters:
        logger.warning(f"Auto-pruning active: Clusters {invalid_clusters} rejected due to physical violations.")

    # Export mapping dataframe
    ...
    rejected_count = int(np.sum(labels == -1))
    logger.info(f"Flagged {rejected_count} curves as structurally anomalous.")
...
if __name__ == "__main__":
    ...
    # Autonomic Self-Healing Loop: Prune non-physical topologies and retrain
    while iteration <= max_iterations:
        ...
        if not invalid_clusters:
            ...
            logger.info("All cluster medoids satisfy physical boundary conditions.")
            break
        
        logger.warning(f"Iteration {iteration} flagged non-physical artifact clusters {invalid_clusters}. Retraining...")
    ...
    if iteration > max_iterations:
        logger.warning("Maximum iterations reached. Serializing last valid state.")

    if ml_results is None:
        logger.error("Pipeline failed to produce valid ML results. Aborting.")
        exit(1)

    logger.info("Executing final projection of topological labels onto the raw dataset...")
